...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Convolution2D, Lambda, concatenate, Input, LeakyReLU
...
class BornAgainModel(object):
    ...
    def prepare(self):
        ...
        x = LeakyReLU(alpha=0.1, name='lrelu1')(x)
        ...
        x = LeakyReLU(alpha=0.1, name='lrelu3')(x)
        ...
        x = LeakyReLU(alpha=0.1, name='lrelu5')(x)
        x = Convolution2D(128, (3, 3), padding='same', name='conv2d6')(x)
        x = BatchNormalization(name='bn6')(x)
        x = LeakyReLU(alpha=0.1, name='lrelu6')(x)
        ...
        x = LeakyReLU(alpha=0.1, name='lrelu8')(x)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher_model_path', type=str, default="teacher_models/teacher_model_epoch_93-val_acc_0.8970000147819519.hdf5")
    ...
    model.train_model.compile(
        optimizer=tensorflow.keras.optimizers.Adam(lr=0.003, beta_1=0.9, beta_2=0.999, epsilon=1e-08),
